...
public class EntityDictionary {
     ...
     * @param entityBean Entity to initialize
     */
    public <T> void initializeEntity(T entityBean) {
        if (entityBean != null) {
            try {
                AccessibleObject initializer = entity(entityBean.getClass()).getInitializer();
                if (initializer instanceof Field) {
                    ((Field) initializer).get(entityBean);
                }
                if (initializer instanceof Method) {
                    ((Method) initializer).invoke(entityBean, (Object[]) null);
                }
